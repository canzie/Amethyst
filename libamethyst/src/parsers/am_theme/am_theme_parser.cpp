    ...
    if (auto arr = node.as_array()) {
        if (arr->size() >= 3) {
            return Color3::fromRgb(
                static_cast<uint8_t>((*arr)[0].value<int64_t>().value_or(0)),
    ...
    for (const auto& [key, value] : section) {
        ...
        if (keyStr == "padding") {
            parseSpacingShorthand(
                value, style, type,
                StyleProperty::PADDING_TOP, StyleProperty::PADDING_RIGHT,
        ...
        if (keyStr == "margin") {
            parseSpacingShorthand(
                value, style, type,
                StyleProperty::MARGIN_TOP, StyleProperty::MARGIN_RIGHT,
                StyleProperty::MARGIN_BOTTOM, StyleProperty::MARGIN_LEFT
            );
            continue;
        }
        if (keyStr == "cellPadding") {
            parseSpacingShorthand(
                value, style, type,
                StyleProperty::CELL_PADDING_TOP, StyleProperty::CELL_PADDING_RIGHT,
