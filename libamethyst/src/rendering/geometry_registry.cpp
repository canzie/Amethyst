...
std::vector<GeometryAllocation *> GeometryRegistry::submitBatch(const std::vector<InstanceData> &dataList)
{
    if (dataList.empty()) return {};

    int32_t zIndex = dataList[0].zIndex;
    ...
    m_allocations.insert(m_allocations.begin() + insertPos, dataList.begin(), dataList.end());

    std::vector<std::unique_ptr<GeometryAllocation>> newHandles;
    newHandles.reserve(count);
    ...
    bucket.count += count;
    for (auto &[bIdx, b] : m_zIndexBuckets) {
        if (bIdx > zIndex && b.count > 0) b.start += count;
    }

    for (uint32_t i = insertPos; i < m_allocations.size(); ++i) {
        m_dirtyIndices.insert(i);
    }

    std::vector<GeometryAllocation *> results;
...
void GeometryRegistry::releaseBatch(std::vector<GeometryAllocation *> &allocs)
{
    ...
    for (auto *alloc : allocs) {
        if (alloc->index != swapTarget) {
            std::swap(m_allocations[alloc->index], m_allocations[swapTarget]);
            std::swap(m_handleMap[alloc->index], m_handleMap[swapTarget]);
            m_handleMap[alloc->index]->index = alloc->index;
        }
        swapTarget--;
    }

    uint32_t eraseStart = bucketEnd - count;
    m_allocations.erase(m_allocations.begin() + eraseStart, m_allocations.begin() + bucketEnd);
    m_handleMap.erase(m_handleMap.begin() + eraseStart, m_handleMap.begin() + bucketEnd);

    for (uint32_t i = eraseStart; i < m_handleMap.size(); ++i) {
        m_handleMap[i]->index = i;
    }
